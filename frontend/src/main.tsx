...
function App() {
  ...
  const generate = async () => {
    setLoading(true);
    const response = await fetch("http://localhost:8000/api/generation/preview", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, columns: selected }) });
    setResult(await response.json()); setLoading(false);
  };
  const download = async () => {
    const response = await fetch("http://localhost:8000/api/export/excel", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, columns: selected }) });
  ...
  return <main><header><p className="eyebrow">Ro-Vibe Dashboard</p><h1>Test data generator</h1><p className="sub">Create realistic distributor, beat, employee, and outlet data in seconds.</p></header>
