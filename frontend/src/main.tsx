...
const generate = async () => {
  try {
    setLoading(true);
    const response = await fetch(`${API_BASE_URL}/api/generation/preview`,{
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...form, columns: selected })
     });
  ...
  const download = async () => {
   ...
   try{
    ...
    link.href = url; link.download = "Generated_Data.xlsx"; 
    link.click(); 
